...
def create_header(token: str | None = None):
    ...
    def _change_lang(new_lang: str):
        if new_lang in ("fr", "no"):
            disclaimer = {
                "fr": "Le texte en français a jusqu'à présent été généré par deepl.com, un outil de traduction basé sur l'IA. Si vous êtes de langue maternelle française et que vous souhaitez relire les 295 chaînes de texte du jeu, n'hésitez pas à nous écrire à l'adresse simfuture@blue-way.net",
                "no": "Den norske teksten er så langt generert av deepl.com, en AI-oversetter. Hvis du har norsk som morsmål og har lyst til å gjennomgå de 295 tekststrengene i spillet, kan du sende oss en e-post til simfuture@blue-way.net",
            }
            with ui.dialog() as dlg, ui.card().classes("max-w-lg"):
                ui.label(disclaimer[new_lang]).classes("text-sm")
                ui.button("OK", on_click=dlg.close).classes("mt-4 self-end")
            dlg.on("hide", lambda: (set_lang(new_lang), db_update_prefs(token, lang=new_lang) if token else None, ui.navigate.reload()))
            dlg.open()
        else:
            set_lang(new_lang)
            if token:
    ...
    with ui.header().classes("items-center justify-between px-4 py-2"):
        ...
        with ui.row().classes("items-center gap-2"):
            ...
            with ui.button(icon='blind',
                ...
                ui.tooltip(luf.help_tip[langx]).classes('bg-green-300 text-black')

            with ui.button(icon='gavel',
                on_click=lambda: ui.navigate.to('/legal')
                        ).props('flat round dense').classes('text-white font-bold'):
                ui.tooltip(luf.legal[langx]).classes('bg-green-300 text-black')
            ...
            with ui.button(
                ...
                ui.tooltip(luf.claude[langx]).classes('bg-green-300 text-black')
            ...
            # Language selector – disabled once inside a game
            in_game = bool(token and session and session.get("game_id"))
            lang_btn = ui.button(LANG_OPTIONS[lang]).props("flat dense").classes("text-white")
            if in_game:
                lang_btn.disable()
                with lang_btn:
                    ui.tooltip(luf.language_locked[langx]).classes("bg-red-300 text-black")
            else:
                with lang_btn:
                    with ui.menu():
...
def home():
    ...
    def get_default_lang() -> str:
        try:
            ...
            # accept-language looks like: "de-DE,de;q=0.9,en;q=0.8"
            primary = accept.split(",")[0].split(";")[0].strip().lower()
            if primary.startswith("de"):
                return "de"
            if primary.startswith("fr"):
                return "fr"
            if primary.startswith("nb") or primary.startswith("no"):
                return "no"
        except Exception:
            pass
        return "en"
...
#    ui.colors(my_orange="#FF8A05")
    ...
    def _open_resume_dialog():
        ...
        with ui.dialog() as dlg, ui.card().classes("p-6 w-80"):
            ui.label(luf.resume_a_game[langx]).classes("text-xl font-bold mb-4")

            gid_input = ui.input(luf.gm_id_title_str[langx], placeholder=luf.ABC123[langx]) \
    ...
    def open_new_game():
        with ui.dialog() as dlg, ui.card().classes("p-6 w-80"):
            ui.label(luf.new_game[langx]).classes("text-xl font-bold mb-4")
            name_input = ui.input(luf.your_name_for_this_game[langx],
                                  placeholder=luf.eg_class_10a[langx]) \
                           .props("autofocus").classes("w-full mb-2")
            ui.label(luf.enter_the_start_code[langx]).classes("text-gray-500 mb-1")
            code_input = ui.input(password=True, placeholder=luf.the_semi_secret_start_code[langx]) \
                           .classes("w-full")
            err_label = ui.label("").classes("text-red-500 text-sm mt-1")
            ...
            def do_new():
                ...
                if not name:
                    err_label.set_text(luf.enter_a_name_for_this_game[langx])
                    return
                if db_find_by_username(name):
                    is_already_taken_choose_another = luf.is_already_taken_choose_another[langx]
            ...
            with ui.row().classes("mt-4 gap-2"):
                ui.button(luf.start[langx], on_click=do_new)
                ui.button(luf.cancel_btn[langx], on_click=dlg.close).props("flat")
    ...
    def open_join():
        with ui.dialog() as dlg, ui.card().classes("p-2 w-96"):
            ...
            def validate():
                ...
                if not name:
                    err_label.set_text(luf.player_join_name[langx])
                    return
                if db_find_by_username(name):
                    is_already_taken_choose_another = luf.is_already_taken_choose_another[langx]
                    err_label.set_text(f"'{name}' {is_already_taken_choose_another}")
                    return
                if not gid:
                    err_label.set_text(luf.please_enter_a_Game_ID[langx])
                    return

                gm = db_find_by_game_id(gid)
                if not gm:
                    game_ug = luf.game_ug[langx]
                    not_found_ug = luf.not_found[langx]
                ...
                region_opts = {a: translated_regs.get(a, a) for a in human_abbrs}

                if check_btn_ref[0]:
                    check_btn_ref[0].set_visibility(False)
                extra.clear()
                with extra:
                    ui.label(luf.select_your_region_and_ministry[langx]) \
                    ...
                    def avail_ministries(region_abbr, players):
                        ...
                        return {m: translated_minis.get(m, m)
                                for m in MINISTRIES if m not in used}
                    ...
                    region_sel   = ui.select(avail_regions(players_now),
                                             label=luf.region[langx],
                                             on_change=on_region_change) \
                                     .classes("w-full")
                    ministry_sel = ui.select([], label=luf.ministry[langx]) \
                                     .classes("w-full mt-2")
                    ...
                    ui.button(luf.join_ug[langx], icon="sports_esports",
                              on_click=do_join, color="positive") \
                      .classes("mt-3 w-full")

            gid_input.on("keydown.enter", validate)
            with ui.row().classes("mt-4 gap-2"):
                check_btn = ui.button(luf.check_game[langx], on_click=validate)
    ...
    lang = get_lang() or get_default_lang()
    langx = LANG_TO_INDEX.get(lang, 0)
    with ui.card().classes("mx-auto mt-16 p-8 w-80"):
        ui.label(luf.welcome[langx]).classes("text-3xl font-bold mb-4")
        ui.button(luf.about_btn_tx[langx], icon='info',
                  on_click=lambda: ui.navigate.to('/about'), color='#FF8F2E').classes('w-full text-black font-bold')
        ui.button(luf.a_fresh_game_start_or_join[langx], icon="add_circle",
                  on_click=open_new_or_join).classes("w-full mb-2")
        if has_resumable:
            ui.label(luf.or_ug[langx]).classes("text-lg font-bold text-center my-2 w-full")
            ui.button(luf.resume_existing_game[langx], icon="list",
    ...
    #         ui.button(luf.refresh[langx], on_click=show_db.refresh).props("flat size=sm")
    #         def clear_all():
    #             with get_db() as conn:
...
def gm_setup(token: str):
    ...
    with ui.column().classes("w-full max-w-2xl mx-auto p-8 gap-4"):
        game_setup = luf.game_setup[langx]
        ui.label(f"{game_setup} · {session['username']}").classes("text-2xl font-bold")
        gm_id_title_str = luf.gm_id_title_str[langx]
        ...
        with ui.card().classes("w-full p-4 bg-amber-50") as card_instructions:
            ui.label(luf.select_human_players[langx]).classes("text-lg font-bold mb-1 text-black")
            hinweise = luf.tick_every_region_that_will_be_played_by_a_human[langx] + "\n" + luf.unticked_regions_are_simulated_by_the_model[langx]
...
#        checkboxes2: dict[str, ui.checkbox] = {}

        with ui.card().classes("w-full p-4 bg-blue-50") as card_select:
            ui.label(luf.regions[langx]).classes("font-bold mb-2 text-black")
...
#                    checkboxes[abbr] = ui.checkbox(name, value=(abbr in human_regions)).classes('text-black').props("dense color=primary keep-color")
                    checkboxes[abbr] = ui.checkbox(regis[abbr], value=(abbr in human_regions)).classes('text-black').props("dense color=primary keep-color")

            def confirm():
                selected = [abbr for abbr, cb in checkboxes.items() if cb.value]
                if not selected:
                    ui.notify(luf.at_least_one_region_must_be_played_by_human_players[langx], color="negative")
                    return
            ...
            ui.button(luf.confirm_selection[langx], icon="check", on_click=confirm).classes("mt-3")
        ...
        with ui.card().classes("w-full p-4 bg-green-50") as card_confirmed:
            ...
            def show_confirmed():
                ...
                ui.label(luf.regions_played_by_human_players[langx]).classes("font-bold mb-2 text-black")
                with ui.row().classes("flex-wrap gap-2 mb-4"):
                    for abbr in selected_abbrs:
                        ui.badge(regis[abbr], color="green")
                with ui.row().classes("gap-2"):
                    ui.button(
                        luf.edit[langx],
                        icon="edit",
                    ...
                    ui.button(
                        luf.continue_to_GM_board[langx],
                        icon="arrow_forward",
    ...
    with ui.column().classes("w-full max-w-4xl mx-auto p-8 gap-4"):
        ...
        with ui.row().classes("w-full items-center justify-between"):
            with ui.column().classes("gap-0"):
                ui.label(luf.gm_board[langx]).classes("text-3xl font-bold")
                game_id_ug = luf.gm_id_title_str[langx]
                your_username = luf.your_username[langx]
                round_ug = luf.runde[langx]
                ui.label(
                    f"{game_id_ug}: {session.get('game_id','?')}  ·  "
                    f"{round_ug} {current_round}/{num_rounds}  · {your_username}: {session['username']}"
                ).classes("text-orange-600 text-xl font-mono font-bold")
        ...
        with ui.card().classes("w-full p-2 bg-amber-50"):
            ...
            def __show_player_ids():
                ...
                with ui.dialog() as dlg, ui.card().classes("w-full max-w-2xl"):
                    ui.label(luf.player_ids[langx]).classes("text-2xl font-bold mb-4")
                    reg_ug = luf.region[langx]
                    mini_ug = luf.ministry[langx]
                    login_id = luf.login_id[langx]
                    if not ps:
                        ui.label(luf.no_players_have_joined_this_game_yet[langx]).classes("text-orange-600")
                    else:
                        ui.table(
                            columns=[
                    ...
                    ui.button(luf.close_ug[langx], on_click=dlg.close).classes("w-full mt-4")
                dlg.open()
            ui.button(luf.show_playerids[langx], icon="badge", on_click=__show_player_ids).props("flat dense size=sm")
            ...
            def _update_labels(rnd, all_joined, all_submitted, done):
                if done:
                    status_label.set_text(luf.game_complete[langx])
                    anweisung_label.set_content(luf.pcgd_rd1_infoend_str[langx])
#                    info_label.set_text("All rounds have been played.")
                elif not all_joined:
                    ui.notify(f'current_round={current_round}',type='warning')
                    runde_ug = luf.runde[langx]
                    waiting_for_players_to_join = luf.waiting_for_players_to_join[langx]
                    status_label.set_text(f"{runde_ug} {rnd} {waiting_for_players_to_join}")
                    if current_round == 1:
                        anweisung_label.set_content(luf.ui_gm_info_start[langx])
                    elif current_round == 2:
                        anweisung_label.set_content(luf.gm_checkbox_open40_tx[langx])
                    elif current_round == 3:
                        anweisung_label.set_content(luf.gm_checkbox_open60_tx[langx])
                    elif current_round == 4:
                        anweisung_label.set_content(luf.gm_checkbox_open21_tx[langx])
                    info_label.set_text(luf.some_regions_have_not_joined_yet[langx])
                elif not all_submitted:
                    runde_ug = luf.runde[langx]
                    waiting_for_submissions = luf.waiting_for_submissions[langx]
                    status_label.set_text(f"{runde_ug} {rnd} {waiting_for_submissions}")
                    anweisung_label.set_content(luf.not_all_submitted[langx])
                    info_label.set_text("")
                else:
                    runde_ug = luf.runde[langx]
                    ready_to_run_the_model_and_advance = luf.ready_to_run_the_model_and_advance[langx]
                    all_submitted = luf.all_submitted[langx]
...
#    print(minis['Food'])
        ...
        with ui.card().classes("w-full p-2 bg-red-50") as card_missing:
            ui.label("[Card 2 – Missing players]").classes("text-xs text-gray-400 italic")
            with ui.row().classes("w-full items-center justify-between mb-1"):
                ui.label(luf.waiting_for_these_ministry_slots_to_join[langx]).classes("font-bold text-black")
            missing_col = ui.column().classes("gap-1")
            def _fill_missing(slots: dict):
                missing_col.clear()
                with missing_col:
                    for a, ministries in slots.items():
                        ui.label(f"• {regis[a]}").classes("text-red-700 font-semibold leading-tight")
                        for m in ministries:
                            ui.label(f"    – {minis[m]}").classes("text-red-600 ml-4 text-sm leading-none")
        ...
        with ui.card().classes("w-full p-4 bg-amber-400") as card_allow_prevent:
            ...
            def _refresh_allow_btn():
                try:
                    ...
                    cur = 0
                if cur == 1:
                    allow_label.set_text(luf.submissions_are_ALLOWED_the_ministers_for_the_Future_can_submit[langx])
                    allow_btn.set_text(luf.prevent_submissions[langx])
                    allow_btn.props("color=negative")
                else:
                    allow_label.set_text(luf.submissions_are_blocked_the_ministers_for_the_Future_can_submit[langx])
                    allow_btn.set_text(luf.allow_submissions[langx])
        ...
        with ui.card().classes("w-full p-4 bg-amber-100") as card_submissions:
            ...
            def _fill_submissions(ps, h_abbrs, gm_round):
                ...
                with sub_col:
                    if post_model:
                        ui.label(luf.waiting_for_players_to_check_results[langx]).classes("text-sm text-gray-500 mb-1")
                    for a in h_abbrs:
                        ...
                        if post_model:
                            ...
                            for m in MINISTRIES:
                                ...
                                with ui.row().classes("items-center gap-1 ml-6"):
                                    ...
                                    ui.label(minis[m]).classes(m_color + " text-sm")
        ...
        with ui.card().classes("w-full p-4 bg-green-100") as card_advance:
            ...
            ui.label(luf.all_regions_have_submitted[langx]).classes("font-bold text-black mb-2")
            ready_to_run_model_and_advance_to_round = luf.ready_to_run_model_and_advance_to_round[langx]
            ...
            def advance():
                ...
                if unsub_now:
                    with ui.dialog() as warn_dlg, ui.card().classes("p-6"):
                        ui.label(luf.not_all_regions_have_submitted[langx]).classes("text-xl font-bold text-red-600 mb-2")
                        for reg, minis in unsub_now.items():
                            ui.label(f"• {abbr_to_name.get(reg, reg)}: {', '.join(minis)}").classes("text-orange-600")
                        ui.button(luf.ok_ug[langx], on_click=warn_dlg.close).props("color=primary")
                    warn_dlg.open()
                    return

                with ui.dialog() as dlg, ui.card().classes("p-6"):
                    ui.label(luf.run_model_advance[langx]).classes("text-xl font-bold mb-2")
                    all_regions_submitted_run_the_simulation_for_round = luf.all_regions_submitted_run_the_simulation_for_round[langx]
                    and_advance_to_round = luf.and_advance_to_round[langx]
                    ui.label(
                    ...
                    with ui.row().classes("gap-2 justify-end mt-4"):
                        ui.button(luf.cancel_btn[langx], on_click=dlg.close).props("flat")
                        ui.button(luf.yes_run_model[langx], icon="play_arrow",
                                  on_click=do_advance).props("color=positive")
                dlg.open()

            run_model_to_round = luf.run_model_to_round[langx]
        ...
        with ui.card().classes("w-full p-4 bg-purple-100") as card_done:
            ...
            ui.label(luf.game_complete[langx]).classes("text-xl font-bold text-purple-700 mb-2")
            ui.label(luf.all_rounds_have_been_played_thank_you_for_playing[langx]).classes("text-sm")
        ...
        def gm_graphs():
            ...
            if current_round == 1:
                luf_head7 = luf.historical_data[langx]
            elif current_round == 2:
                luf_head7 = luf.hist_2040[langx]
            elif current_round == 3:
                luf_head7 = luf.hist_2060[langx]
            elif current_round == 4:
                luf_head7 = luf.hist_2100[langx]
            else:
                        ...
                        with ui.row().classes("w-full gap-4"):
                            ...
                            with ui.card().classes("lg:flex-1 w-full p-4"):
                                if img:
                                    ...
                                    ui.label(luf.graph_data_unavailable[langx]).classes("text-sm text-orange-400 italic")
                            if i + 1 < len(graphs):
                                pv, img = graphs[i + 1]
                                with ui.card().classes("lg:flex-1 w-full p-4"):
                                    if img:
        ...
        with ui.card().classes("w-full p-2 bg-stone-50") as card_refresh:
            ...
            ui.button(luf.refresh_btn[langx], icon="refresh", on_click=refresh).props("flat")
...
def _check_results_btn(token: str, game_id: str, current_round: int, region: str):
    ...
    ui.button(luf.check_if_results_are_ready[langx], icon="refresh",
...
def _render_graphs(game_id: str, region: str, role: str, lang: str, langx: int,
    ...
    try:
        ...
        no_graph_data_available = luf.no_graph_data_available[langx]
...
#    ui.notify(f'runde={runde} actual_runde={actual_runde}')
    if actual_runde == 0:
        lufhead = luf.historical_data[langx]
        lufinfo = luf.pcgd_rd1_info_tx_str[langx]
    elif actual_runde == 1:
        lufhead = luf.hist_2040[langx]
        lufinfo = luf.pcgd_rd1_info_tx_str[langx]
    elif actual_runde == 2:
        lufhead = luf.hist_2060[langx]
        lufinfo = luf.pcgd_rd1_info_tx_str[langx]
    elif actual_runde == 3:
        lufhead = luf.hist_2100[langx]
        lufinfo = luf.pcgd_rd1_infoend_str[langx]
    else:
        lufhead = f'ERROR: actual_runde={actual_runde}'
        lufinfo = f'ERROR lufinfo: actual_runde={actual_runde}'
        
    if game_data is None:
        ui.label(luf.no_graph_data_available[langx]).classes("text-gray-400 italic")
        return

    try:
        ministry_label = luf.minis[lang][role]
    except Exception:
        ministry_label = role
    ...
    with ui.card().classes("w-full p-6"):
        ...
        ui.label(
            f"{luf.monitoring[langx]}{len(plot_vars)}{luf.indicator_for[langx]}{ministry_label}"
        ).classes("text-orange-500 font-bold mb-4")
        ui.markdown(lufinfo)
                ...
                with ui.row().classes("w-full gap-4"):
                    ...
                    with ui.card().classes("lg:flex-1 w-full p-4"):
                        if img:
                            ...
                            ui.label(pv.get("pv_indicator", "")).classes("text-lg font-bold")
                            ui.label(luf.no_graph_data_available[langx]).classes("text-sm text-orange-400 italic")
                    if i + 1 < len(graphs):
                        pv, img = graphs[i + 1]
                        with ui.card().classes("lg:flex-1 w-full p-4"):
                            if img:
...
def _render_sliders(token: str, game_id: str, current_round: int,
    ...
    round_ranges = [luf.now_2040, luf.to_2060, luf.to_2100]
    rrange = round_ranges[min(current_round - 1, 2)][langx]

    lang = get_lang()
    langx = LANG_TO_INDEX.get(lang, 0)
    try:
        pol_expls = luf.pols_expl[lang]
    except Exception:
        pol_expls = {}
    ...
    def slider_section():
        ...
        with ui.card().classes("w-full p-4"):
            ...
            ui.label(f"{luf.pol_decs[langx]} – {display_region} – {minis[role]}") \
              .classes("text-2xl font-bold mb-2")
            ui.label(luf.dec_title_tx_str[langx] + str(current_round) + rrange) \
              .classes("text-lg text-orange-700 mb-4")
            ui.markdown(luf.dec_info_tx_str[langx])

            if not policies:
                ui.label(f"ERROR: No policies assigned to {role}").classes("text-red-600")
                return
            ...
            for policy in policies:
                ...
                pol_name_translated = luf.pol_names.get(lang, luf.pol_names["en"]).get(
                    pol_tag, policy["pol_name"])
                with ui.card().classes("w-full mb-1 bg-gray-50").tight():
                    exp = ui.expansion(pol_name_translated, icon="info").classes("w-full font-bold")
...
def _render_budget(token: str, game_id: str, current_round: int,
    ...
    def budget_section():
        ...
        with ui.card().classes("w-full p-2 gap-2"):
            ...
            ui.label(f"{luf.budget_considerations[langx]}, {display_region}") \
              .classes("text-2xl font-bold")
            ui.label(luf.bud_title_tx_str[langx] + str(current_round) + rrange) \
              .classes("text-blue-600 text-lg")
            ui.markdown(luf.every_10_sec[langx]) \
...
#            ).props("color=primary")
            ...
            def _submit_proposals():
                try:
                    ...
                    allowed = 1
                if allowed == 0:
                    ui.notify(luf.ooops_GM_changed_mind[langx],
                              type="negative", position="center")
                    return
                with ui.dialog() as confirm_dlg, ui.card():
                    ui.label(luf.confirm_submission[langx]).classes("text-xl font-bold mb-2")
                    ui.label(luf.are_you_sure_you_want_to_submit_all_investment_proposals[langx])
                    ui.label(f"{luf.budget2[langx]}: {bud:.1f}")
                    ui.label(f"{luf.total2[langx]}: {grand_total[0]:.1f}")
                    ui.label(luf.once_submitted[langx]).classes("text-orange-600 mt-2")
                    with ui.row().classes("w-full justify-end gap-2 mt-4"):
                        ...
                        ui.button(luf.yes_submit[langx], on_click=_do_submit).props("color=primary")
                poll_paused[0] = True
                confirm_dlg.open()
                    ...
                    if all_joined and not not_checked and grand_total[0] <= bud:
                        with ui.card().classes("w-full bg-green-50 p-1"):
                            ui.label(
                                ...
                                f"{luf.all_investment_plans_summed_up_for_your_region[langx]} {grand_total[0]:.1f}"
                        ...
                        with ui.card().classes("w-full bg-orange-50 p-1"):
                            ui.label(
                                ...
                                f"{luf.plans2[langx]}{bud:.1f}{luf.plans3[langx]}"
...
def dashboard(token: str):
    ...
    langx = LANG_TO_INDEX.get(lang, 0)
    if not session:
        ui.label(luf.session_not_found__redirecting[langx]).classes("text-red-500 m-8")
    ...
    translated_regs  = luf.regs.get(lang, luf.regs["en"])
    translated_minis = luf.minis.get(lang, luf.minis["en"])
    display_region   = translated_regs.get(region, region)
    display_role     = translated_minis.get(role, role)
    ...
    with ui.column().classes("w-full max-w-6xl mx-auto p-8 gap-6"):
        ...
        with ui.row().classes("w-full items-center justify-between"):
            with ui.column().classes("gap-1"):
                ...
                ui.label(f"{display_region} – {display_role}").classes("text-2xl text-orange-600")
                game_ug = luf.game[langx]
                runde_ug = luf.runde[langx]
                ui.label(
...
def about_page(token: str = ""):
    ...
    with ui.column().classes('w-full max-w-4xl mx-auto p-8 gap-6 items-center'):
        ...
        ui.markdown(luf.about_md[langx]).classes('w-full')
        # Back button
        ui.button(luf.btn_back[langx], 
...
def legal_page(token: str = ""):
    ...
    with ui.column().classes('w-full max-w-4xl mx-auto p-8 gap-6 items-center'):
        # Centered markdown content
        ui.markdown(luf.legal_md[langx]).classes('w-full')
        # Back button
        ui.button(luf.btn_back[langx], 
                 on_click=lambda: ui.navigate.back(),
                 icon='arrow_back').classes('mt-4')
