...
async def detect_browser_settings():
    ...
    if app.storage.user.get('lang') is None:
        app.storage.user['langx'] = langx
        app.storage.user['lang'] = lang
    
    return lang, langx

def set_mode_btn(d):
    is_dark = do_dark_light()
#    print(is_dark)
    if not is_dark:
        app.storage.user['dark_mode'] = 'true'
        ui.dark_mode().enable()
        print(f'set_mode_btn: {is_dark} not dark : setting to dark')
    else:
        app.storage.user['dark_mode'] = 'false'
        ui.dark_mode().disable()
        print(f'set_mode_btn: {is_dark} dark : setting to light')
    ui.navigate.reload()
...
#    await ui.context.client.connected()
    deff = app.storage.user.get('dark_mode')
#    print(f'do_dark_light, deff={deff}')
    if deff == None:
        print(f'do_dark_light None, setting to system' )
        mode = ui.dark_mode()
        if mode == 'light':
            ui.dark_mode().disable()
            app.storage.user['dark_mode'] = 'false'
        ...
        if app.storage.user['dark_mode'] == 'true':
            ui.dark_mode().enable()
            return True
        else:
            ui.dark_mode().disable()
...
def create_theme_toggle_button():
    ...
    current_mode = do_dark_light()
#    print(f'create_theme_toggle_button: dark is {current_mode}')
    ...
    def toggle_theme():
        current_mode_dark = do_dark_light()
#        print(f'toggle_theme: dark is {current_mode_dark}')
...
#        await ui.context.client.connected()
        with ui.row().classes('gap-2 items-center'):
            ...
            langx = app.storage.user.get('langx', 0)
            if langx == 0:
           ...
           # Language selector
            ...
            current_lang = app.storage.user.get('lang', 'en')
...
def home():
    ...
    last = state.resume_or_sign_in(token)
    if last and last != '/':
    ...
    def do_login(username, game_id, role, ministry, region_tag):
        state.sign_in(token, username, game_id, role, ministry, region_tag)
...
#     create_header()
    ...
    with ui.column().classes('w-full max-w-2xl mx-auto p-8 gap-6'):
        ui.markdown(luf_original.sdg_first_page[app.storage.user.get('langx', 0)]).classes('w-full text-3xl font-bold text-center')
        ui.button(luf_original.about_btn_tx[app.storage.user.get('langx', 0)], icon='about',
        ...
        with ui.card().classes('w-full p-6'):
            ui.label(luf_original.game_master[app.storage.user.get('langx', 0)]).classes('text-2xl font-bold mb-4')
            
            with ui.row().classes('w-full gap-4'):
                ui.button(luf_original.start_new_game[app.storage.user.get('langx', 0)], 
                         on_click=lambda: ui.navigate.to('/gm/new'),
                         icon='add_circle').classes('flex-1')
                
                ui.button(luf_original.continue_existing_game[app.storage.user.get('langx', 0)], 
                         on_click=lambda: ui.navigate.to('/gm/continue'),
                         icon='play_arrow').classes('flex-1')
        
        # Player Section
        with ui.card().classes('w-full p-6'):
            ui.label(luf_original.IamaPlayer[app.storage.user.get('langx', 0)]).classes('text-2xl font-bold mb-4')
            
            with ui.row().classes('w-full gap-4'):
                ui.button(luf_original.joining[app.storage.user.get('langx', 0)], 
                         on_click=lambda: ui.navigate.to('/player/join'),
                         icon='person_add', color='secondary').classes('flex-1')
                
                ui.button(luf_original.rejoining[app.storage.user.get('langx', 0)], 
...
def gm_new_game():
    ...
    with ui.column().classes('w-full max-w-2xl mx-auto p-8 gap-6'):
        ui.label(luf_original.start_new_game[app.storage.user.get('langx', 0)]).classes('text-3xl font-bold')
        
        with ui.card().classes('w-full p-6'):
            start_code_input = ui.input(luf_original.enter_code_title_tx[app.storage.user.get('langx', 0)],
                                       placeholder=luf_original.enter_code_tx[app.storage.user.get('langx', 0)],
                                       password=True,
            ...
            username_input = ui.input(luf_original.your_username_game_master[app.storage.user.get('langx', 0)],
                                     placeholder=luf_original.please_enter_a_username[app.storage.user.get('langx', 0)]).on('keydown.enter', lambda: create_game()).classes('w-full')
            ...
            def create_game():
                ...
                if start_code != db.START_CODE:
                    error_label.text = luf_original.wrong_code_tx[app.storage.user.get('langx', 0)]
                    return
                
                if not username:
                    error_label.text = luf_original.please_enter_a_username[app.storage.user.get('langx', 0)]
                    return
                
                # Check if username is available globally (including as GM)
                player_info = db.get_player_info(username)
                if player_info:
                    error_label.text = luf_original.username_already_taken_please_choose_another[app.storage.user.get('langx', 0)]
                    return
                ...
                with db.get_db() as conn:
                    ...
                    if cursor.fetchone():
                        error_label.text = luf_original.username_already_taken_as_gm_please_choose_another[app.storage.user.get('langx', 0)]
                        return
                
                game_id = db.create_game(username)
                state.sign_in(token, username, game_id, 'GM', 'GM', None)
                ui.navigate.to(f'/gm/config?token={token}')
            
            ui.button(luf_original.create_game[app.storage.user.get('langx', 0)], on_click=create_game, icon='create').classes('w-full')
        
        ui.button(luf_original.btn_back[app.storage.user.get('langx', 0)], on_click=lambda: ui.navigate.to('/'), 
...
# ============================================================================
@ui.page('/gm/config')
def gm_config_page(token: str):
    do_dark_light()
    create_header()
    session = state.require_role(token, 'GM')
    if not session:
        return
    state.save_last_page(token, '/gm/config')
    ...
    state_x = game_info['state_x']
    print(f'gm/config: {game_id} {username} | {lang} state={state_x} round={current_round}')
...
#                ui.label(luf.ai_controlled_regions[langx]).classes('text-xl font-bold mb-2')
...
#        ui.button(luf.back_to_home[langx], on_click=lambda: ui.navigate.to('/'), 
...
def gm_board(token: str):
    ...
    print(f'gm/board | game_info: {lang} {str(state_x)} {str(current_round)}')
    ...
    with ui.column().classes('w-full max-w-6xl mx-auto p-8 gap-6'):
        ...
        with ui.row().classes('w-full items-center justify-between'):
            with ui.column():
                ...
                ui.label(luf_original.game[langx]+'-ID '+game_id+' | '+luf_original.runde[langx]+': '+str(current_round)+'/'+str(num_rounds)).classes('text-xl text-orange-600 font-mono')
...
async def player_join_game():
    ...
    with ui.column().classes('w-full max-w-2xl mx-auto p-8 gap-6'):
        ...
        with ui.card().classes('w-full p-6'):
            ...
            # todo DOUBLE CHECK if still available
                player_in_db = db.does_player_exist(username)
                if player_in_db:
                    ui.notify('This usernames exists after all ...', type='negative',position='center')
                    return
                # Update session
                db.set_game_info_player(username, game_id, 'player' ,lang, langx)
                
                # Navigate to region selection
                ui.navigate.to(f'/player/select_region?token={token}&username={username}&game_id={game_id}')
    ...
    with ui.column().classes('w-full max-w-2xl mx-auto p-8 gap-6'):
        ...
        with ui.card().classes('w-full p-6'):
            ...
            async def check_and_resume():
                ...
                ministry = player['ministry']
                state.sign_in(token, username, player['game_id'], ministry, ministry, region_tag)
                if ministry == 'Future':
                    ui.navigate.to(f'/future/dashboard?token={token}')
                else:
                    ui.navigate.to(f'/player/dashboard?token={token}')
...
def player_select_region(token: str, username: str, game_id: str):
    do_dark_light()
    create_header()
    if not token or not username or not game_id:
        ui.navigate.to('/')
        return
    langx = app.storage.user.get('langx', 0)
    lang  = app.storage.user.get('lang', 'en')
                        ...
                        for ministry in ministries:
                            def make_select_handler(r_tag, m):
                                def handler():
                                    ...
                                    if success:
                                        state.sign_in(token, username, game_id, m, m, r_tag)
                                        if m == 'Future':
                                            ui.navigate.to(f'/future/dashboard?token={token}')
                                        else:
                                            ui.navigate.to(f'/player/dashboard?token={token}')
...
def player_dashboard(token: str):
    session = state.require_role(token, 'Poverty', 'Inequality', 'Empowerment',
                                        'Food', 'Energy', 'Future')
    if not session:
        return
    state.save_last_page(token, '/player/dashboard')
    username   = session['username']
    game_id    = session['game_id']
...
#                ui.label(f'Game: {game_id} | Round: {current_round}').classes('text-lg text-gray-600 font-mono')
        ...
        @ui.refreshable
        def render_results():
            res_data, res_runde = game_plot_ug.load_game_data(game_id, current_round)
            if res_runde >= 1:
                with ui.expansion(
        ...
        # ===================================================================
        _, init_runde = game_plot_ug.load_game_data(game_id, current_round)
        ...
        def _check_results():
            _, runde = game_plot_ug.load_game_data(game_id, current_round)
...
def future_dashboard(token: str):
    do_dark_light()
    create_header()
    session = state.require_role(token, 'Future')
    if not session:
        return
    state.save_last_page(token, '/future/dashboard')
    username   = session['username']
    game_id    = session['game_id']
    region_tag = session['region_tag']
    ministry   = session['ministry']
...
def about_page():
    ...
    with ui.column().classes('w-full max-w-4xl mx-auto p-8 gap-6 items-center'):
        ...
        ui.markdown(luf_original.about_md[app.storage.user.get('langx', 0)]).classes('w-full')
        # Back button
        ui.button(luf_original.btn_back[app.storage.user.get('langx', 0)], 
...
def legal_page():
    ...
    with ui.column().classes('w-full max-w-4xl mx-auto p-8 gap-6 items-center'):
        # Centered markdown content
        ui.markdown(luf_original.legal_md[app.storage.user.get('langx', 0)]).classes('w-full')
        # Back button
        ui.button(luf_original.btn_back[app.storage.user.get('langx', 0)], 
                 on_click=lambda: ui.navigate.back(),
                 icon='arrow_back').classes('mt-4')
